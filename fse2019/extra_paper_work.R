...
THE_FILE <- "/Users/akond/Documents/AkondOneDrive/OneDrive/ProcessInIaC/output/FSE2019/extra-work/minors-cutoff-minmax.csv"
#xintercept: devs  = 12, minors  = 8, 
THE_DAT  <- read.csv(THE_FILE)
print(head(THE_DAT))
pdf('/Users/akond/Documents/AkondOneDrive/OneDrive/ProcessInIaC/writing/minors-max-max.pdf', width=6, height=3)
...
line_plot <- line_plot + scale_x_continuous(breaks=seq(1, 40, 5))
line_plot <- line_plot + labs(x='Minor contributors')
line_plot <- line_plot + theme_classic()
line_plot <- line_plot + theme(legend.position="bottom")
line_plot <- line_plot + theme( text = element_text(size=20), axis.text=element_text(size=20))
...
line_plot <- line_plot + geom_vline(xintercept = 8, linetype="twodash", color = "magenta", size=0.5) #reff: http://www.sthda.com/english/wiki/ggplot2-add-straight-lines-to-a-plot-horizontal-vertical-and-regression-lines
line_plot
